...
- The next implementation critical path starts at `T-1007` for the remaining Discord bot/RCON work; after that, operator-facing player-count and browser-console tasks continue at `T-1010` through `T-1012`.
...
- `T-1007` is in progress: Rails now has the bot-side API skeleton under `/api/discord/bot/*`, including bot bearer auth, acting Discord-user resolution, Docker-private-network route gating, policy-checked status/lifecycle/whitelist endpoints, and owner/admin-only bounded RCON commands.
