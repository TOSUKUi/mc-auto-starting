...
- `T-1110` also fixes two implementation constraints: startup settings should be editable from browser detail and Discord bot surfaces, and enum-like settings such as `difficulty` / `gamemode` should default to `Select` controls instead of freeform text.
- `T-1111` through `T-1113` are complete: `minecraft_servers` now persists baseline startup settings, the create/detail browser flows expose them with `Select` controls for enum-like values plus explicit `hardcore`, and the internal Discord bot API can read/update the same desired-state contract.
- `T-1114` is in progress: the current desired-state contract is being reconsidered so create remains the source of initial defaults while detail shifts toward read-only startup-settings display plus structured allowlisted RCON actions for mutable live server properties.
- `T-1115` follows that with a repository-wide audit of active input validation paths to ensure browser-visible validation is always backed by controller/model/service rejection on the server side.
- `T-1116` through `T-1119` are now queued as the next RCON-UX follow-up: mutable live-setting changes should stay out of DB-backed desired state, while detail and bot move toward a shared structured `command_key + args` catalog with schema-driven forms and optional-target patterns such as `gamemode(mode, player_name?)`.
- `T-1116` is complete: `docs/structured_rcon_command_catalog.md` now fixes the shared structured RCON command catalog and argument schema, including optional-target patterns such as `gamemode(gamemode, player_name?)`, before backend and UI implementation continue.
...
- `T-1120` is queued: whitelist detail UX should move above server operations, use a toggle-style mode switch, warn strongly when enabled with zero entries, keep a persistent warning while disabled, and guide the user toward the existing add-player section instead of embedding another form into the warning.
- `T-1121` is queued: the index `詳細を見る` affordance needs a list/card UX pass so the primary action is clearly visible and not confined to a small right-edge button.
- The current startup-settings detail presentation now separates real-time vs restart-applied behavior explicitly, while the initial baseline keeps all editable startup settings in the restart-applied group because these settings still apply on the next start/restart rather than via live RCON mutation.
