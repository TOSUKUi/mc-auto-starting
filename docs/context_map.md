...
- `T-503` is in progress: operator-facing copy and layout polish are being moved further toward a Japanese-first, simpler presentation; the unused home page has been removed in favor of routing `/` to the server index, the active app shell now uses a flat Minecraft-inspired dark theme, the header now exposes the signed-in user's global role, shared navigation/member/invite copy is being localized, the server create form caps memory at 4GB while blocking invalid hostname characters earlier, the default runtime choice is now `Java Edition`, create-page validation errors now stay visible after 422 responses, the invite screen has started dropping repeated helper text per the active audit rules, the membership page now follows the same hero/card hierarchy as the other active screens, the server index now exposes an explicit `詳細を見る` action instead of relying on the name link alone, the create screen is continuing to drop self-evident helper text while tightening the confirmation block, the detail screen is trimming redundant helper copy while moving startup-settings save actions into the card header, and the shared shell/index/invite surfaces are continuing to lose self-evident helper text and decorative filler.
