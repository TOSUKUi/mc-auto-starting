...
- `T-1114` is now the next startup-settings follow-up: the current desired-state contract is being reconsidered so create remains the source of initial defaults while detail shifts toward structured allowlisted RCON actions for mutable live server properties.
