...
- `T-1105` through `T-1107` are complete: the create UI resolves runtime-family-specific version choices on the Rails side when the page opens, caches them briefly, falls back to the checked-in catalog, and links each runtime family to its upstream version source without exposing a freeform version field.
