...
- `T-1119` is complete: the bot-side RCON endpoint now uses the same structured `command_key + args` contract as the browser, rejects raw command fallback server-side, and returns `command_key` plus the built command through the shared validation path.
