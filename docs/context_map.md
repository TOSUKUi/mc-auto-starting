...
- `T-1116` through `T-1119` are now queued as the next RCON-UX follow-up: mutable live-setting changes should stay out of DB-backed desired state, while detail and bot move toward a shared structured `command_key + args` catalog with schema-driven forms and player-target patterns such as `gamemode(mode, player_name)`.
