...
- 初期版の実行イメージは `marctv/minecraft-papermc-server` 系を前提とする。
...
- Minecraft コンテナ:
  - 実サーバープロセス
  - コンテナ単位で `marctv/minecraft-papermc-server` を実行
...
`marctv/minecraft-papermc-server:<minecraft_version>` コンテナへ最低限渡すもの:

- `MEMORYSIZE=<memory_mb から 512MB 引いた JVM heap>`
- `PAPERMC_FLAGS=`
...
`itzg/minecraft-server` 系では、Minecraft version は image tag と同一視せず、公式 docs の `VERSION` 環境変数契約を正本にする。
