...
## Next Layout Cleanup Plan
今回のレビューで、一覧/詳細に残っている route 系メタデータと情報重複を次の見直し対象として固定する。
...
### De-duplicate Next
- 詳細上段と中段で重複している `現在の状態`
- 詳細上段の接続先カードと中段の `接続先` / `アドレス`
- 詳細上段の runtime `Type` と中段の `種類`
- 詳細上段の `最終起動` / `連続稼働時間` カードと中段の同項目

### Rebalanced Layout Goal
- 上段: サーバー名、接続先、状態、主要操作
- 中段: Minecraft バージョン、種類、オーナー、アクセス権
- 補助情報: hostname、fqdn、最終起動、連続稼働時間
- トラブル時のみ強調: 直近エラー
