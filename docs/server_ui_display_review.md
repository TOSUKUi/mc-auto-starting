...
- startup settings の編集は owner / admin のみ可能にする
- startup settings の変更は desired state として保存し、次回の起動または再起動で反映する
- 詳細の startup settings は「いま反映できる設定」と「再起動で反映する設定」に分けて見せる
- 初期 baseline では startup settings 全体が再起動反映なので、editable controls は再起動反映セクションへ置く
