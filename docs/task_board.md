...
| T-1114 | P10 | Reframe startup settings as create-time defaults and move detail changes to structured RCON actions | T-1110,T-1111,T-1112,T-1012 | in_progress | Startup settings remain part of create-time/default runtime config, detail no longer treats Rails desired state as the live source of truth for mutable server properties, owner/admin detail actions move toward structured allowlisted RCON forms instead of a freeform console, and the current detail screen now shows startup settings as read-only initial values alongside structured RCON controls such as difficulty/weather/time/say/kick/save-all |
...
| T-1116 | P10 | Define structured RCON command catalog and argument schema | T-1114,T-1012 | todo | Detail and bot RCON operations use a shared command catalog with `command_key + args` payloads, including optional-target schemas such as `gamemode(mode, player_name?)`, and mutable live settings are not persisted as Rails desired state |
| T-1117 | P10 | Add Rails-side structured RCON command builder and validation | T-1116 | todo | Rails converts `command_key + args` into bounded RCON commands server-side, validates required/optional arguments, and keeps mutable live-setting changes out of DB-backed desired-state storage |
| T-1118 | P10 | Replace detail RCON cards with command-select plus argument form UI | T-1116,T-1117 | todo | Server detail exposes one structured RCON surface with a command select, schema-driven argument inputs, and result rendering instead of separate action cards or freeform command input |
