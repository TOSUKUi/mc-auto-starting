...
- `T-503`: layout/create/index/detail copy is being shifted further toward Japanese, active server screens now hide Docker backend identifiers behind simpler player-facing status and connection language, and the app shell has moved to a flat Minecraft-inspired dark theme with `/` routed directly to the server index.
