...
| T-907 | P8 | Audit and remove obsolete environment keys after runtime/deploy cleanup | T-903,T-906,T-1101,T-1104 | done | `.env.example`, `compose.yaml`, `config/deploy.yml`, initializers, and env-contract docs agree on the actively supported keys only; dead keys such as stale bootstrap leftovers are removed; and runtime-image overrides that no longer make sense after the `itzg`-only `paper` / `vanilla` baseline are either deleted or explicitly justified |
| T-908 | P8 | Collapse fixed-value env knobs into code and reclassify required app secrets | T-907,T-1003,T-1007 | done | Fixed router/runtime/socket/RCON defaults now live in code or checked-in Compose/Kamal config instead of `.env`, Discord OAuth keys are treated as current steady-state requirements, the internal bot API uses `DISCORD_BOT_API_TOKEN` instead of unused bot-process secrets, and operator docs/template files agree on the reduced env surface |
...
- `T-907` is complete: obsolete env keys have been pruned after the runtime-family and Kamal decisions settled, including removal of the dead `BOOTSTRAP_EMAIL_ADDRESS` compose pass-through and the now-unneeded `MINECRAFT_RUNTIME_VANILLA_IMAGE` override so the supported contract now reflects the shared `itzg` baseline.
- `T-908` is complete: fixed-value env knobs for Docker transport, runtime catalog sources, `mc-router` reload wiring, network naming, and RCON transport defaults have been moved out of operator-facing env, while Discord OAuth is now treated as a current required app secret and the bot-facing app secret is consistently `DISCORD_BOT_API_TOKEN`.
