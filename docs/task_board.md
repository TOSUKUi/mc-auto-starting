...
| T-503 | P4 | Localize operator-facing UI copy to Japanese baseline | T-500,T-501,T-502 | in_progress | Default operator-facing copy is Japanese across the active screens, the create flow now defaults to `Java Edition` with Japanese-first runtime labels, and create-form validation errors render back into the active page instead of disappearing on 422 |
...
| T-1111 | P10 | Persist baseline startup settings alongside server desired state | T-1110,T-400 | todo | Rails can store the agreed baseline settings such as `hardcore`, `difficulty`, `max_players`, `motd`, `pvp`, and `gamemode` as desired server state shared by create, detail, and bot surfaces |
| T-1112 | P10 | Add startup settings UI to create/detail flows | T-1110,T-1111,T-500,T-501 | todo | Operators can set the agreed baseline startup settings during create and adjust them later from server detail, with enum-like settings rendered as `Select` controls and `hardcore` exposed explicitly |
