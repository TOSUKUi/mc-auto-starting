...
| T-1023 | P9 | Add whitelist management UI to the server detail page | T-1021,T-1022,T-505 | done | Authorized users can view and edit desired whitelist state on server detail, running servers apply changes immediately through RCON, stopped servers stage changes that are applied on the next start, the detail page does not loop whitelist fetches, and whitelist mutations re-sync only the card while safely handling non-JSON restart responses |
