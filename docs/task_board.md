...
| T-1025 | P9 | Make whitelist live-apply and restart reconciliation reliable end-to-end | T-1021,T-1023 | in_progress | RCON auth/command-response handling and restart-time container recreation are being tightened so running-server whitelist mutations can be verified end-to-end and restart/start both apply the current saved whitelist env before the final saved-but-not-live-applied failure contract is closed |
