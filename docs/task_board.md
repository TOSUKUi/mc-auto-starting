...
| T-503 | P4 | Localize operator-facing UI copy to Japanese baseline | T-500,T-501,T-502 | done | Default operator-facing copy is Japanese across the active screens, the create flow defaults to `Java Edition` with Japanese-first runtime labels, create-form validation errors render back into the active page instead of disappearing on 422, startup-settings presentation is grouped and simplified on create/detail, login now uses the same visual system as the authenticated shell while keeping only the page label plus a short service summary and a `Discordでログイン` action, index exposes an explicit `詳細を見る` action, and the create/detail/index/invite/membership/login surfaces have shed most self-evident helper text under the `docs/ui_polish_audit_strategy.md` audit |
