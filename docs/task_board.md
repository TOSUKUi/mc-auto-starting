...
| T-1015 | P9 | Add global user-type model for `admin` / `operator` / `reader` | T-1014,T-101 | todo | `users` carry the new global type and authorization code can resolve it independently from server membership |
| T-1016 | P9 | Restrict invitation authority by user type | T-1014,T-1002,T-1015 | todo | Admins can invite without restriction, operators can invite only readers, and readers cannot issue invites |
| T-1017 | P9 | Enforce create authorization by user type at controller/policy level | T-1014,T-1015,T-107,T-404 | todo | Admins can always create, operators are quota-limited, readers are denied, and controller-level authorization rejects unauthorized requests |
| T-1018 | P9 | Enforce reader read-only access across web and bot surfaces | T-1014,T-1015,T-1005,T-1010 | todo | Readers can access read-only views/endpoints while lifecycle, invite, and command surfaces are rejected at controller/policy level |
