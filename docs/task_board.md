...
| T-508 | P4 | Remove duplicated route metadata and rebalance server layouts | T-503,T-505,T-506,T-507 | todo | Server index/detail no longer surface `応答状態`, `最終反映`, or `最終ヘルスチェック`, repeated facts such as status/connection/runtime labels are shown only once per screen, and the primary detail layout clearly separates connection, action, ownership, and lower-priority technical metadata |
...
- `T-508`: the next server-screen cleanup should remove `応答状態` plus router timestamp noise from both index/detail, eliminate repeated facts between the summary cards and lower detail sections, and rebalance the detail page so connection, action, ownership, and troubleshooting information have clearer separation.
