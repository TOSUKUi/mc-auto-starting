...
| T-1021 | P9 | Add Rails-side whitelist command service over RCON | T-1006,T-1020 | done | Rails can inspect and mutate server whitelist entries through bounded RCON commands with timeout/error handling and no direct container-console exposure |
...
| T-1024 | P9 | Align future bot command scope with whitelist authority | T-1005,T-1020,T-1021 | todo | The bot trust boundary explicitly covers which whitelist read/write actions are allowed and keeps them inside the Rails-owned RCON surface |
