...
| T-1025 | P9 | Make whitelist live-apply and restart reconciliation reliable end-to-end | T-1021,T-1023 | todo | Running-server whitelist changes succeed only after verified RCON command/response completion, stopped-server desired state is persisted for the next boot, restart/start paths both recreate the container with current whitelist env, and failures return a clear saved-but-not-live-applied message |
