...
| T-503 | P4 | Localize operator-facing UI copy to Japanese baseline | T-500,T-501,T-502 | in_progress | Default operator-facing copy is Japanese across the active screens, the create flow now defaults to `Java Edition` with Japanese-first runtime labels, create-form validation errors render back into the active page instead of disappearing on 422, startup-settings presentation is being cleaned up with grouped create-form sections plus explicit restart-applied messaging on detail, and the remaining whole-app polish is now being audited under `docs/ui_polish_audit_strategy.md` |
| T-512 | P4 | Audit all active pages against the UI polish strategy | T-503 | todo | Create/detail/membership/login/invite and the remaining active pages are reviewed against `docs/ui_polish_audit_strategy.md`, with a concrete per-page keep/remove/unify list before more UI edits land |
| T-513 | P4 | Redesign Discord login to match the app shell visual system | T-503,T-512,T-1003 | todo | `/login` uses the same visual system as the authenticated control plane, keeps only minimal Discord/invite guidance, and no longer feels like a separate product surface |
