...
- `T-1105` / `T-1106` / `T-1107`: create UI now resolves `vanilla` and `paper` version choices on the Rails side during page load, caches them briefly, falls back to the checked-in catalog, and links each runtime family to its upstream version source without a freeform version field.
