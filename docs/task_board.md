...
| T-804 | P7 | Verify live `mc-router` ingress against managed containers | T-303,T-400,T-803 | todo | A real `mc-router` service can load generated routes, reach `mc-server-<hostname>:25565` on `mc_router_net`, and accept an end-to-end connection on the shared public port |
...
`T-110 -> T-200 -> T-201 -> T-202 -> T-203 -> T-204 -> T-300 -> T-301 -> T-302 -> T-303 -> T-304 -> T-400 -> T-402 -> T-500 -> T-501 -> T-803 -> T-804 -> T-900 -> T-901 -> T-902`
...
- `T-804`: live ingress verification still needs a real `mc-router` service; current checks only prove route file generation plus direct container reachability.
