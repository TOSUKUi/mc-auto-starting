...
| T-503 | P4 | Localize operator-facing UI copy to Japanese baseline | T-500,T-501,T-502 | in_progress | Default operator-facing copy is Japanese across the active screens, the create flow now defaults to `Java Edition` with Japanese-first runtime labels, create-form validation errors render back into the active page instead of disappearing on 422, startup-settings presentation is being cleaned up with grouped create-form sections plus explicit restart-applied messaging on detail, the whole-app polish is being tracked under `docs/ui_polish_audit_strategy.md`, the invite surface has started shedding repeated helper text in line with that audit, the membership page now uses the same hero/card hierarchy as the other active screens, the index now exposes an explicit `詳細を見る` action instead of relying on the server name link alone, the create screen is dropping self-evident helper text while tightening the confirmation block, the detail screen is trimming redundant helper copy while moving startup-settings save actions into the card header, and the shared shell/index/invite surfaces are continuing to lose self-evident helper text and decorative filler |
