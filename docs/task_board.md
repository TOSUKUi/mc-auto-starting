...
| T-1020 | P9 | Define whitelist and global ingress restriction contract | T-1000,T-1006,T-1014 | done | `docs/whitelist_and_access_control_strategy.md` fixes Rails-owned RCON whitelist operations, their authority boundary, and the global-only `mc-router` IP allow/deny scope before implementation |
| T-1021 | P9 | Add Rails-side whitelist command service over RCON | T-1006,T-1020 | todo | Rails can inspect and mutate server whitelist state through bounded RCON commands with timeout/error handling and no direct container-console exposure |
| T-1022 | P9 | Add authorization and request/service tests for whitelist operations | T-1021,T-1018 | todo | Only admins and owners can mutate whitelist state, stopped-server and RCON-failure handling are covered, and unauthorized users are rejected at controller/policy level |
| T-1023 | P9 | Add whitelist management UI to the server detail page | T-1021,T-1022,T-505 | todo | Authorized users can view running-server whitelist state and perform add/remove/on/off/reload actions with clear success/failure feedback |
| T-1024 | P9 | Document and wire global `mc-router` client IP allow/deny config | T-1020,T-901,T-904 | todo | Compose/Kamal/operator docs expose host-wide `CLIENTS_TO_ALLOW` / `CLIENTS_TO_DENY` handling without presenting it as a per-server control |
| T-1025 | P9 | Align future bot command scope with whitelist authority | T-1005,T-1020,T-1021 | todo | The bot trust boundary explicitly covers which whitelist read/write actions are allowed and keeps them inside the Rails-owned RCON surface |
...
- `T-904`: `docs/kamal_deployment_topology.md` now fixes the single-host Kamal shape: one `web` app role, MariaDB and Redis as Kamal accessories, `mc-router` as a long-lived sibling service, untracked `.kamal` secret files, and the deploy-time env mapping from local `.env`.
...
- `T-1101`: create flow now exposes `runtime_family` as a user-facing choice, persists it through the legacy `template_kind` column for now, keeps `paper` as the default, and provisions both runtime families through `itzg/minecraft-server`.
