...
- `T-503` is complete: operator-facing UI copy and layout now follow a simpler Japanese-first presentation across the active path; `/` lands on the server index, the active app shell uses the flat Minecraft-inspired dark theme, the header shows the signed-in user's global role, shared navigation/member/invite labels are Japanese-first, current server forms enforce a 4GB memory cap plus hostname character restrictions in both JS and Rails validations, the create flow defaults to `Java Edition`, create-page validation errors render back into the active Inertia form on 422, the create/detail/index/invite/membership/login surfaces have shed self-evident helper text, the login page now keeps only the page label plus a short service summary and a single `Discordでログイン` action, the index exposes an explicit `詳細を見る` action, and detail-page startup-settings save actions now live in the card header instead of a duplicated footer action.
- The next `T-503` UI pass is now scoped by `docs/ui_polish_audit_strategy.md`, which defines the page-by-page audit across login, create, detail, membership, invite, and the rest of the active path before more visual edits land.
- `T-512` is complete: the concrete keep/remove/unify inventory for login, create, detail, membership, invite, index, and the shared shell now lives in `docs/ui_polish_audit_inventory.md` and should be consulted before further UI polish work.
- `T-513` is complete: `/login` now matches the authenticated control-plane palette and card system, keeps only minimal Discord/invite guidance, and no longer uses the earlier split-screen auth-only design.
- Inertia-driven logout and unauthenticated redirects now use `X-Inertia-Location` when sending the browser to the non-Inertia `/login` page, so session-expiry/logout no longer render login as a modal-like invalid Inertia response.
...
- After the Discord bot/RCON track, the remaining visible UI polish work is centered on `T-503`.
- `T-1101` is complete: create flow now exposes runtime family selection with `Java Edition` (`vanilla`) as the default, and both `paper` and `vanilla` provision through the `itzg/minecraft-server` runtime family.
...
- `T-1111` through `T-1113` are complete: startup settings now persist on `minecraft_servers`, flow into the managed runtime env contract, can be set during create and adjusted later from server detail by owner/admin users, remain visible to other visible members as read-only detail data, and are exposed through the internal Discord bot API.
- `T-1114` is in progress: startup settings are being reconsidered as create-time defaults only, detail has started shifting from owner/admin desired-state editing to read-only initial-settings display plus structured allowlisted RCON actions, and the remaining follow-up is to finish aligning the broader startup-settings/bot contract with that direction.
...
- `T-1116` is complete: the shared structured RCON catalog now lives in `docs/structured_rcon_command_catalog.md`, fixing the initial `command_key + args` schema for browser and bot surfaces, including optional-target commands such as `gamemode(gamemode, player_name?)`.
...
- `T-1120` is queued: the whitelist card should move above generic server operations, use a toggle-style mode control, warn strongly when enabled with zero entries, keep a persistent warning while disabled, and guide the user toward the existing add-player section instead of embedding another form into the warning.
- `T-1121` is queued: the server index `詳細を見る` action needs a UX re-audit so the primary path to detail is larger, more stable, and easier to scan than the current small right-edge control.
- Startup-settings detail UI now explicitly separates real-time vs restart-applied behavior at the presentation layer; the current baseline keeps the editable controls under a restart-applied section because startup settings still save desired state for the next start/restart rather than applying live through RCON.
