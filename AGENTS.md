...
- `T-1025` is in progress: raw-socket verification showed this Minecraft runtime returns a single auth packet, so RCON auth now uses `ignore_first_packet: false`; `WhitelistManager` now reads response bodies instead of object inspection, `restart` now stops/removes/recreates/starts the managed container so saved whitelist env is reapplied, and live verification against `muuchannel` confirmed both `WHITELIST=TOSUKUi2` and `/data/whitelist.json` were updated.
