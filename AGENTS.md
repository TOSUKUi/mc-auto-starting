...
- `T-503` is in progress: operator-facing UI copy and layout polish are being shifted toward a simpler Japanese-first presentation; the root route now lands on the server index, the active app shell uses a flat Minecraft-inspired dark theme, the header now shows the signed-in user's global role, shared navigation/member/invite labels are Japanese-first, current server forms enforce a 4GB memory cap plus hostname character restrictions in both JS and Rails validations, the create flow now defaults to `Java Edition` instead of `Paper`, create-page validation errors now render back into the active Inertia form instead of disappearing on 422 responses, and the invite screen has started shedding repeated helper text in line with the active audit rules.
