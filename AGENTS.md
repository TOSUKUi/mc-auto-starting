...
- `T-1007` is in progress: Rails now has an internal-only `/api/discord/bot/*` surface gated to the Docker private network plus a dedicated bot bearer token, and the current implementation already covers acting-user resolution, policy-checked status/lifecycle/whitelist endpoints, and owner/admin-only bounded RCON commands.
