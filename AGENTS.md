...
- `T-503` is in progress: operator-facing UI copy and layout polish are being shifted toward a simpler Japanese-first presentation; the root route now lands on the server index, the active app shell uses a flat Minecraft-inspired dark theme, the header now shows the signed-in user's global role, shared navigation/member/invite labels are Japanese-first, current server forms enforce a 4GB memory cap plus hostname character restrictions in both JS and Rails validations, the create flow now defaults to `Java Edition` instead of `Paper`, create-page validation errors now render back into the active Inertia form instead of disappearing on 422 responses, the invite screen has started shedding repeated helper text in line with the active audit rules, the membership page now uses the same hero/card hierarchy as the other active screens, the index now exposes an explicit `詳細を見る` action instead of relying on the server name link alone, the create screen is dropping self-evident helper text while compressing the confirmation block into a denser summary card, the detail screen is trimming redundant helper copy while moving startup-settings save actions into the card header, and the shared shell/index/invite surfaces are continuing to lose self-evident helper text and decorative filler.
