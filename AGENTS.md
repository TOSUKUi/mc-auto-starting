...
4. Critical path work is blocked on `T-300`; if continuing implementation without that dependency, move to `T-602` for the server index page UI while keeping Rails built-in authentication and Pundit as the current baseline.
